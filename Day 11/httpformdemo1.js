...
const server=http.createServer(function(req,resp){
    //parse the url and seperatethe url and data in the query string
    var q=url.parse(req.url,true)////true:(to parse the query string as an object).
    //handle all the possible urls
    resp.writeHeader(200,{"content-type":"text/html"})
    switch(q.pathname){
        case "/form":
            var rs=fs.createReadStream("./public/form.html");
            //The pipe method is used to redirect the data from the read stream to the response stream.
            rs.pipe(resp); 
            break;
        case "/submit-data":
            //console.log(q);
